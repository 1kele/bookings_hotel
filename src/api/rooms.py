...
async def get_rooms(
    ...
    return await db.rooms.get_rooms_filter_by(
...
async def update_room(hotel_id: int, db: DBDep, room_id: int, data: RoomAddRequest):
    ...
    await db.rooms.edit(new_data, id=room_id)
...
async def delete_room(hotel_id: int, room_id: int, db: DBDep):
    try:
        ...
        raise HTTPException(
            status_code=409, detail="Нельзя удалить комнату - она связана с удобствами"
        )
    return {"status": "Ok"}
